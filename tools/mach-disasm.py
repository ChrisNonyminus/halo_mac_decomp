...
bssAddress = 0x00432a60 # Sims 2 BV: 0x02b92560 # Halo: 0x00432a60 # Sims 2: 0x022d6638 
bssSize = 0x000ae818 # Sims 2 BV: 0x003027fc # Halo: 0x000ae818 # Sims 2: 0x00278d90
...
def insn_to_text(insn, raw):
    ...
    if insn.address in splitDataLoads and insn.id == PPC_INS_LIS:
        ...
        suffix = 'ha16'
        return '%s %s,%s(%s)' % (insn.mnemonic, insn.reg_name(insn.operands[0].reg), suffix, addr_to_label(value))
    # Handle split loads (low part)
    elif insn.address in splitDataLoads and insn.id in {PPC_INS_ADDI, PPC_INS_ORI}:
        value = splitDataLoads[insn.address]
        return '%s %s,%s,lo16(%s)' % (insn.mnemonic, insn.reg_name(insn.operands[0].reg), insn.reg_name(insn.operands[1].reg), addr_to_label(value))
    elif insn.address in splitDataLoads and is_load_store_reg_offset(insn, None):
        value = splitDataLoads[insn.address]
        return '%s %s,lo16(%s)(%s)' % (insn.mnemonic, insn.reg_name(insn.operands[0].reg), addr_to_label(value), insn.reg_name(insn.operands[1].mem.base))
...
def disassemble_callback(address, offset, insn, bytes):
    ...
    if address in labels:
        if address in labelNames:
            print("\n.globl %s" % addr_to_label(address))
        print("%s: ;# %s" % (addr_to_label(address), demangle(addr_to_label(address))))
